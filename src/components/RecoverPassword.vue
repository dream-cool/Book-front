    ...
    <div style="width:35%;margin-left:20%;margin-top:5%">
      ...
      <div>
        ...
        <div class="first" v-if="active == 0">
          ...
          <br>
            <el-button @click="getUserInfo" :disabled="sendButtonMessageDisabled" style="margin-top: 50px;"> {{sendButtonMessage}} </el-button>
        ...
        <div class="second" v-if="active == 1">
          ...
          <br>
          <el-button @click="commitNewPW"  style="margin-top: 50px;"> 提交 </el-button>
        </div>
      </div>
    ...
    getUserInfo(){
        ...
        axios.get('/user/queryUser/'+ this.user.userName +'/' + this.user.email).then(res => {
          if(res.data.code === 200){
            ...
            this.$message.error('没有找到用户信息，或绑定邮箱不正确')
          }
        })
    }
  }
