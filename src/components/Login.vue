<template>
  <div class="hello" :style="backgroundDiv">
    <el-dialog :lock-scroll="false" title="励新图书管理系统" width="30%" :modal="false" :show-close="false"
      ...
      <el-form :model="user" :rules="userRules" ref="userRules" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item>
          ...
          <el-link style="marginLeft:50px;marginTop: -5px" type="primary"
                   :underline="false" @click="recoverPasswor">找回密码
          </el-link>
          <!-- <el-link style="marginLeft:50px;marginTop: -5px" type="primary"
            ...
            axios.get('/login?userName=' + this.user.userName + '&password=' + this.user.password).then(
              res => {
                if (res.data.code == 200 && (res.data.data.userDetail.role == '2' || res.data.data.userDetail.role == '3')) {
                  ...
                  if (res.data.data.userDetail.role == '1' || res.data.data.userDetail.role == '0') {
