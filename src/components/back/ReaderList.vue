  ...
  <div class="content">
      <el-container>
          ...
          <el-form-item label="信用大于" prop="price">
            <el-input-number v-model="user.price" controls-position="right"
                :min="0" :max="100"></el-input-number>
          </el-form-item>
          <el-form-item label="性别" prop="author">
            <el-input v-model="user.author" placeholder="模糊查询书籍作者"></el-input>
          </el-form-item>
          <el-form-item label="用户状态" prop="status">
            <el-select v-model="user.userStatus" placeholder="书籍状态">
          ...
          <el-form-item label="角色" prop="ebook">
...
export default {
  ...
  methods: {
    ...
    handleDelete (row) {
      this.$confirm('确认删除？')
        .then(_ => {
          axios.delete('/user/' + row.bookId).then(res => {
            if (res.data.code === 200) {
              this.$message(res.data.message)
              this.getBookInfo(this.pageNum, this.pageSize, this.book)
            } else {
