<template>
    ...
    <el-table-column id="el-table-column2" align="left" header-align="left" label="操作" width="400px" v-if="operations != null">
      <template id="el-free-tag14" slot-scope="{row,$index}">
          <el-button id="el-button10" v-if="showEditButton(operation, $index)" v-for="(operation,index) in operations" :key="index" @click.native="executeOperation(operation,$index, row)" :round="operation.round" :size="operation.size" :plain="operation.plain" :disabled="operation.disabled" :type="operation.type" :circle="operation.circle" :loading="operation.loading" :autofocus="operation.autofocus" :icon="operation.icon">
          {{operation.text}}
          </el-button>
          <el-button id="el-button11" v-if="showSaveButton(operation, $index)" v-for="(operation,index) in operations" :key="index" @click.native="executeOperation(operation,$index, row)" :round="operation.round" :size="operation.size" :plain="operation.plain" :disabled="operation.disabled" :type="operation.type" :circle="operation.circle" :loading="operation.loading" :autofocus="operation.autofocus" :icon="operation.icon">
          {{operation.text}}
          </el-button>
      </template>
    </el-table-column>
